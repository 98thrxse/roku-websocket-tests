...
function Logger() as object
    ...
    ' Parse Config
    config_string = readAsciiFile("pkg:/configs/bws.json")
    ...
    if config <> invalid
        if config.log_level <> invalid
            ...
            log.printl(log.WARN, "WebSocketLogger: Missing log_level param in pkg:/configs/bws.json")
        end if
    else
        log.log_level = log.INFO
        log.printl(log.WARN, "WebSocketLogger: Missing pkg:/configs/bws.json")
    end if
    return log
end function
